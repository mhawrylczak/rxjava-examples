...
class BackpressureMaxConcurrentTransformerSync<T, R> implements Observable.Transformer<T, R> {
    private final Func1<T, Observable<R>> transformer;
    private final long maxConcurrent;

    public static <T, R> BackpressureMaxConcurrentTransformerSync<T, R> create(Func1<T, Observable<R>> transformer, long maxConcurrent) {
        return new BackpressureMaxConcurrentTransformerSync<>(transformer, maxConcurrent);
    }

    private BackpressureMaxConcurrentTransformerSync(Func1<T, Observable<R>> transformer, long maxConcurrent) {
